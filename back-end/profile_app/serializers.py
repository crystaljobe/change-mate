...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    def get_events_attending(self, obj):
        ...
        ser_events_attending = [{'id':x.id, 'title':x.title, 'date':x.date, 'time':x.time, 'event_type':x.event_type, 'event_venue':x.event_venue, 'event_venue_address':x.event_venue_address, 'virtual_event_link':x.virtual_event_link, 'description':x.description, 'users_attending':x.users_attending.count()} for x in events_attending]
        return ser_events_attending

    def get_user_events(self, obj):
        user_events = obj.user_events.all()
        ser_user_events = [{'id':x.id, 'title':x.title, 'date':x.date, 'time':x.time, 'event_type':x.event_type, 'event_venue':x.event_venue, 'event_venue_address':x.event_venue_address, 'virtual_event_link':x.virtual_event_link, 'description':x.description,'users_attending':x.users_attending.count()} for x in user_events]
